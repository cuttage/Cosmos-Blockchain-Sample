...
import { BaseVestingAccount } from "cuttage-cosmoschain-client-ts/cosmos.vesting.v1beta1/types"
import { ContinuousVestingAccount } from "cuttage-cosmoschain-client-ts/cosmos.vesting.v1beta1/types"
import { DelayedVestingAccount } from "cuttage-cosmoschain-client-ts/cosmos.vesting.v1beta1/types"
import { Period } from "cuttage-cosmoschain-client-ts/cosmos.vesting.v1beta1/types"
import { PeriodicVestingAccount } from "cuttage-cosmoschain-client-ts/cosmos.vesting.v1beta1/types"
import { PermanentLockedAccount } from "cuttage-cosmoschain-client-ts/cosmos.vesting.v1beta1/types"
...
export default {
	...
	actions: {
		...
		async sendMsgCreatePermanentLockedAccount({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.CosmosVestingV1Beta1.tx.sendMsgCreatePermanentLockedAccount({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgCreatePeriodicVestingAccount({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.CosmosVestingV1Beta1.tx.sendMsgCreatePeriodicVestingAccount({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
