...
import { Plan } from "cuttage-cosmoschain-client-ts/cosmos.upgrade.v1beta1/types"
import { SoftwareUpgradeProposal } from "cuttage-cosmoschain-client-ts/cosmos.upgrade.v1beta1/types"
import { CancelSoftwareUpgradeProposal } from "cuttage-cosmoschain-client-ts/cosmos.upgrade.v1beta1/types"
import { ModuleVersion } from "cuttage-cosmoschain-client-ts/cosmos.upgrade.v1beta1/types"
	...
	actions: {
		...
		async sendMsgSoftwareUpgrade({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.CosmosUpgradeV1Beta1.tx.sendMsgSoftwareUpgrade({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
