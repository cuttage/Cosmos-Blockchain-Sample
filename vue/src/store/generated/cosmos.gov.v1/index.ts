...
import { WeightedVoteOption } from "cuttage-cosmoschain-client-ts/cosmos.gov.v1/types"
import { Deposit } from "cuttage-cosmoschain-client-ts/cosmos.gov.v1/types"
import { Proposal } from "cuttage-cosmoschain-client-ts/cosmos.gov.v1/types"
import { TallyResult } from "cuttage-cosmoschain-client-ts/cosmos.gov.v1/types"
import { Vote } from "cuttage-cosmoschain-client-ts/cosmos.gov.v1/types"
import { DepositParams } from "cuttage-cosmoschain-client-ts/cosmos.gov.v1/types"
import { VotingParams } from "cuttage-cosmoschain-client-ts/cosmos.gov.v1/types"
import { TallyParams } from "cuttage-cosmoschain-client-ts/cosmos.gov.v1/types"
	...
	actions: {
		...
		async QueryProposals({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.CosmosGovV1.query.queryProposals({...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
		...
		async QueryVotes({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.CosmosGovV1.query.queryVotes( key.proposal_id, {...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
					value = mergeResults(value, next_values);
				}
		...
		async QueryDeposits({ commit, rootGetters, getters }, { options: { subscribe, all} = { subscribe:false, all:false}, params, query=null }) {
			try {
				...
				while (all && (<any> value).pagination && (<any> value).pagination.next_key!=null) {
					let next_values=(await client.CosmosGovV1.query.queryDeposits( key.proposal_id, {...query ?? {}, 'pagination.key':(<any> value).pagination.next_key} as any)).data
					value = mergeResults(value, next_values);
				}
		...
		async sendMsgVoteWeighted({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				...
				const result = await client.CosmosGovV1.tx.sendMsgVoteWeighted({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
		...
		async sendMsgVote({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.CosmosGovV1.tx.sendMsgVote({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgSubmitProposal({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.CosmosGovV1.tx.sendMsgSubmitProposal({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
		...
		async sendMsgDeposit({ rootGetters }, { value, fee = [], memo = '' }) {
			try {
				const client=await initClient(rootGetters)
				const result = await client.CosmosGovV1.tx.sendMsgDeposit({ value, fee: {amount: fee, gas: "200000"}, memo })
				return result
			} catch (e) {
				if (e == MissingWalletError) {
